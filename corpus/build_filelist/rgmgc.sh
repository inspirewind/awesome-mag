...
# input files: downloads/rgmgc/assemblies/*_genomic.fna.gz
# output filelist: corpus/cluster_inputs/rabbittclust/rgmgc.list
#
# RGMGC MAG assemblies are fetched from NCBI BioProject PRJNA657473 as one
# gzip FASTA per assembly. RabbitTClust can read gzip FASTA, so this builder
# writes direct absolute paths without extraction.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
INSPECT_ONLY=0
GZIP_TEST=0
EXPECTED_FASTA_COUNT=10373
LOG_EVERY=1000
...
expected_fasta_count = int(os.environ.get("EXPECTED_FASTA_COUNT", "10373"))
...
ncbi_manifest = download_dir / "ncbi_assemblies_manifest.tsv"
...
members_tsv = debug_dir / "rgmgc.members.tsv"
...
if zero_size:
    ...
    raise SystemExit(f"{len(zero_size)} RGMGC FASTA files are zero-size:\n{preview}")

if expected_fasta_count and len(fastas) != expected_fasta_count:
    raise SystemExit(
        f"Unexpected RGMGC FASTA count: {len(fastas)} != {expected_fasta_count}. "
...
if inspect_only:
    ...
    print(f"ncbi_manifest: {ncbi_manifest if ncbi_manifest.exists() else 'missing'}")
